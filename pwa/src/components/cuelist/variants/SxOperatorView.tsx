...
import { soundPayloadSummary } from '../payloadSummaries.js';
import { tokens } from '../tokens.js';

interface SxOperatorViewProps {
  cuelistId: string;
  watched: string[];
}

const OWNED_SET = new Set(['SX']);

export function SxOperatorView({ cuelistId, watched }: SxOperatorViewProps) {
      ...
      >
        {visible.map((c) => (
          <OperatorCueRow
            ...
            owned={OWNED_SET}
            extraColumns={[{ label: 'Sound', content: soundPayloadSummary(c) }]}
            goLabel={goLabel}
            onGo={() => { void go(c.id); }}
