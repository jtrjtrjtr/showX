...
import { fsPositionSummary } from '../payloadSummaries.js';
import { tokens } from '../tokens.js';

interface FsOperatorViewProps {
  cuelistId: string;
  watched: string[];
}

const OWNED_SET = new Set(['FS']);

export function FsOperatorView({ cuelistId, watched }: FsOperatorViewProps) {
    ...
    >
      <header
        style={{
          ...
          background: tokens.color.dept['FS'],
          color: tokens.color.ink,
        }}
      >
        <h2 style={{ margin: 0, fontSize: 18 }}>FOLLOWSPOT</h2>
      </header>
      <div
        role="grid"
        aria-label="FS operator view"
      ...
      >
        {visible.map((c) => (
          <OperatorCueRow
            ...
            owned={OWNED_SET}
            extraColumns={[{ label: 'Position', content: fsPositionSummary(c) }]}
            goLabel={goLabel}
            onGo={() => { void go(c.id); }}
