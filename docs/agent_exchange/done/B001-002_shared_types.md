...
- `OutputDispatcher` — `send(msg: TransportMessage): Promise<DispatchResult>`, `claim(dest: TransportDestination): Promise<ClaimToken | ClaimConflict>`, `release(token: ClaimToken): Promise<void>`, `poolStatus(): PoolStatus`. Note: `claim` and `release` are async because they may open/close OS-level resources (UDP sockets, MIDI ports) — sync signature would deadlock the Electron main thread (Q30 ruling 2026-06-05).
